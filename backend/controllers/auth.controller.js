...
const google = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      res.status(200).json({ token, ...user._doc }); // Send token and user data in response
    } else {
      ...
      res.status(200).json({ token, ...newUser._doc }); // Send token and new user data in response
    }
  } catch (error) {
    next(error);
  }
